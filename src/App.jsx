...
function ContattiScreen({ data, setData, civico, via, onSelect, onBack, onUfficio, user }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const fmt = `${now.getDate()}/${now.getMonth()+1}/${now.getFullYear()} ${String(now.getHours()).padStart(2,"0")}:${String(now.getMinutes()).padStart(2,"0")}:${String(now.getSeconds()).padStart(2,"0")}`;
      const tempId = Date.now();
      const nuoveAttivita = form.risposto === "Y"
        ? [{ id: tempId + 1, contattoId: tempId, commento: "RISPOSTO", dataOra: fmt, utente: user?.email || "" }]
        : [];
      setData(d => ({ ...d,
        contatti: [...(d.contatti||[]), { id: tempId, ...form, nome: form.nome.toUpperCase(), cognome: form.cognome.toUpperCase(), civicoId: civico?.id }],
        attivita: [...(d.attivita||[]), ...nuoveAttivita],
      }));
    }
