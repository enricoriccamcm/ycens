...
function ZoneScreen({ data, setData, ufficio, onSelect, onBack, onUfficio }) {
  ...
  const add = () => {
    if (!nome.trim()) return;
    setData(d => ({ ...d, zone: [...d.zone, { id: d.nextId.zone, nome: nome.trim().toUpperCase(), ufficioId: ufficio.id }], nextId: { ...d.nextId, zone: d.nextId.zone + 1 } }));
...
function VieScreen({ data, setData, zona, onSelect, onBack, onUfficio }) {
  ...
  const add = () => {
    if (!nome.trim()) return;
    setData(d => ({ ...d, vie: [...d.vie, { id: d.nextId.vie, nome: nome.trim().toUpperCase(), zonaId: zona.id }], nextId: { ...d.nextId, vie: d.nextId.vie + 1 } }));
    setNome(""); setModal(false);
  };
  return (
    <div style={{ minHeight: "100vh", background: T.bg, paddingBottom: 58 }} className="screen">
...
function CiviciScreen({ data, setData, via, zona, onSelect, onBack, onUfficio }) {
  ...
  const add = () => {
    ...
    setData(d => ({ ...d, civici: [...d.civici, { id: d.nextId.civici, numero: numero.trim(), viaId: via.id }], nextId: { ...d.nextId, civici: d.nextId.civici + 1 } }));
...
function ContattiScreen({ data, setData, civico, via, onSelect, onBack, onUfficio, user }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      // fallback local
      setData(d => ({ ...d, contatti: [...d.contatti, { id: d.nextId?.contatti || Date.now(), ...form, nome: form.nome.toUpperCase(), cognome: form.cognome.toUpperCase(), civicoId: civico.id }], nextId: { ...d.nextId, contatti: (d.nextId?.contatti||0) + 1 } }));
    }
...
function ContattoScreen({ data, setData, contatto: contattoInit, civico, onBack, onUfficio, user }) {
  ...
  const addAtt = () => {
    ...
    setData(d => ({ ...d, attivita: [...d.attivita, { id: d.nextId.attivita, contattoId: contatto.id, commento: commento.trim(), dataOra: fmt, utente: user.email }], nextId: { ...d.nextId, attivita: d.nextId.attivita + 1 } }));
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}
            onClick={() => {
              ...
              const commento = contatto.risposto === "Y" ? "NON RISPOSTO" : "RISPOSTO";
              const now = new Date();
              const fmt = `${now.getDate()}/${now.getMonth()+1}/${now.getFullYear()} ${String(now.getHours()).padStart(2,"0")}:${String(now.getMinutes()).padStart(2,"0")}:${String(now.getSeconds()).padStart(2,"0")}`;
              setData(d => ({
                ...d,
                contatti: d.contatti.map(x => x.id === contatto.id ? { ...x, risposto: nuovoStato } : x),
                attivita: [...d.attivita, { id: d.nextId.attivita, contattoId: contatto.id, commento: commento, dataOra: fmt, utente: user?.email || "" }],
                nextId: { ...d.nextId, attivita: d.nextId.attivita + 1 }
              }));
            }}
